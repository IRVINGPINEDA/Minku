...
public class UserController {
    ...
    @PostMapping("/lesson/{id}/view")
    public ResponseEntity<?> registerLessonView(@PathVariable Long id) {
        Lesson lesson = lessonRepository.findById(id).orElse(null);
        if (lesson == null)
            return ResponseEntity.notFound().build();
        lesson.setViews(lesson.getViews() == null ? 1L : lesson.getViews() + 1);
        lessonRepository.save(lesson);
        return ResponseEntity.ok(lesson.getViews());
    }
